...
普通上传数据集在“训练设置”中设置“异构程度（Dirichlet α）”，默认 `0.01`，范围 `0.0001–100`；α 越小，客户端类别分布通常越不均衡。划分种子默认为 `42`。切换训练算法时保留 α 和划分种子，便于在同一数据划分上比较方法。

划分仅针对训练集：按类别采样客户端占比，并将每张训练图恰好分配给一个客户端；测试集和显式 train/test 清单不变。极小 α 导致空客户端时，从最大客户端移动一张样本以确保客户端非空。一个客户端时无法形成客户端间异构。配置、实际客户端类别计数、划分指纹随实验保存；图像特征缓存可复用，但客户端划分须重新预检。此协议不会修改已完成实验的结果；旧实验若重新运行会采用新协议，应先重新预检。
