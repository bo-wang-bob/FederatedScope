...
export const pages = {
  ...
  backdoor: { label: '后门防御', section: '研究扩展' },
  backdoorCompare: { label: '逐样本对照', section: '研究扩展' },
  ...plannedModules,
} as const;
...
export function mainView(view: PlatformView): MainView | 'privacy' | 'backdoor' {
  return view === 'jobs' ? 'train' : view === 'evaluate' ? 'experience' : view === 'backdoorCompare' ? 'backdoor' : view as MainView | 'privacy' | 'backdoor';
}
...
export const backdoorCompareHref = (job?: string | null) => '/?' + new URLSearchParams({ view: 'backdoorCompare', ...(job ? { job } : {}) });
