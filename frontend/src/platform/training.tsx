...
export function TrainingForm({ catalog, initialGroup, sourceId, running, disconnected, launch, open }: {
  ...
  return <div className="training-wizard studio-page-enter">
    <aside className="wizard-rail"><h2>实验配置</h2>
      ...
      <div className="wizard-draft-status"><i />{saved ? '本地草稿已保存' : '浏览器未允许保存草稿'}</div>
    ...
    <section className="wizard-panel" aria-label="训练配置">
      ...
      {step === 0 && <><div className="wizard-title"><span>步骤 01</span><h2>数据与算法</h2></div><div className="wizard-fields">
        <div className="wizard-field"><label htmlFor="train-name">实验名称<span className="field-optional">可选</span></label><Input id="train-name" value={draft.name} maxLength={120} placeholder="输入实验名称" disabled={frozen} onChange={event => change('name',event.target.value)} /></div>
      ...
      {step === 1 && <><div className="wizard-title"><span>步骤 02</span><h2>训练参数</h2><div className="wizard-selection">{group?.dataset} <i> / </i> {methodLabel(draft.method)}</div></div>
      ...
      {step === 2 && <><div className="wizard-title"><span>步骤 03</span><h2>{draft.name || '确认实验配置'}</h2></div>
