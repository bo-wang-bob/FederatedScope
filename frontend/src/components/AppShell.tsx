...
export function AppShell({ children, platform }: PropsWithChildren<{ platform?: PlatformShellState }>) {
  ...
  const navigation = <>
    ...
    <Menu mode="inline" selectedKeys={[selectedKey]} items={menuItems} onClick={({ key }) => { (platform?.navigate || navigate)(key); setNavigationOpen(false); }} className="side-menu" />
    <div className="sidebar-status">
      ...
      {platform && <div className="engine-row"><span>10.112.81.135</span></div>}
      <div className="engine-row"><span>执行方式</span><Tag color="cyan">{platform ? '单机联邦' : 'API'}</Tag></div>
      <div className="engine-row"><span>逻辑客户端</span><strong>{platform ? platform.clientCount ?? '—' : 60}</strong></div>
      <div className="engine-row"><span>数据域</span><strong>{platform ? platform.domainCount ?? '—' : 4}</strong></div>
    </div>
  </>;
  ...
  return <ConfigProvider theme={{ algorithm: theme.darkAlgorithm, token: { colorPrimary: '#39c9ff', colorBgBase: '#07101f', colorBgContainer: '#0e1c2d', colorBorder: 'rgba(125, 177, 220, .16)', borderRadius: 8, fontFamily: "Inter, 'PingFang SC', 'Microsoft YaHei', sans-serif" } }}>
    <Layout className={`app-layout ${immersive ? 'immersive-layout' : ''}`}>
      ...
      <Layout>
        {!immersive && <Header className="app-header"><div className="header-context"><span className="context-dot" /><span className="context-name">跨域联邦学习统一实验平台</span><Tag bordered={false} color="cyan">{platform ? '准确率实验 · 缓存只读' : '真实任务控制'}</Tag></div>{platform?.openCurrent ? <Button type="link" onClick={platform.openCurrent}>打开当前实验 →</Button> : !platform && activeExperimentId && <Button type="link" onClick={() => navigate(monitorPath)}>打开当前实验</Button>}</Header>}
