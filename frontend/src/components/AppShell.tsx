...
export function AppShell({ children }: PropsWithChildren) {
  ...
  const navigation = <>
    ...
    <div className="sidebar-status">
      <div className="sidebar-status-head"><span>统一实验引擎</span><Badge status="processing" /></div>
  ...
  return <ConfigProvider theme={{ algorithm: theme.darkAlgorithm, token: { colorPrimary: '#39c9ff', colorBgBase: '#07101f', colorBgContainer: '#0e1c2d', colorBorder: 'rgba(125, 177, 220, .16)', borderRadius: 8, fontFamily: "Inter, 'PingFang SC', 'Microsoft YaHei', sans-serif" } }}>
    <Layout className={`app-layout ${immersive ? 'immersive-layout' : ''}`}>
      ...
      <Layout>
        {!immersive && <Header className="app-header"><div className="header-context"><span className="context-dot" /><span className="context-name">跨域联邦学习统一实验平台</span><Tag bordered={false} color="cyan">真实任务控制</Tag></div>{activeExperimentId && <Button type="link" onClick={() => navigate(monitorPath)}>打开当前实验</Button>}</Header>}
