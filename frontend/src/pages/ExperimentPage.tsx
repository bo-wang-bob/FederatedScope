...
import type { BackdoorConfig, Capabilities, ExperimentConfig, ExperimentMethod, ExperimentMode, PrivacyConfig } from '../types';
...
export function ExperimentPage() {
  ...
  const config = useMemo<ExperimentConfig>(() => {
    ...
    return {
      ...
      heterogeneity: { expansionTarget },
      privacy: null, backdoor: null,
    };
  }, [backdoorAttack, backdoorDefense, batchSize, device, epsilon, expansionTarget, featureDefense, fedproxMu, initialClip, learningRate, localEpochs, maliciousClients, maliciousRatio, method, mode, name, noiseMultiplier, participationRate, poisonRatio, privacyAttack, privacyDefense, rounds, scenarioId, seed, startRound, targetLabel, targetQuantile, trainingDefense]);
  ...
  const startExperiment = async () => {
    ...
    try {
      ...
      await experimentApi.preflight(request);
  ...
  return <div className="page experiment-page">
    ...
    <div className="experiment-layout">
      ...
      <aside className="experiment-summary"><Panel title="运行摘要" subtitle="后端提交前校验"><div className="summary-scene"><span>基础异构环境</span><Tag color={scenarioId ? 'cyan' : 'warning'}>{scenarioId ? scenarioVersion || scenarioId : '尚未应用'}</Tag></div><div className="summary-list"><div><span>实验类型</span><b>{modeCards.find((item) => item.key === mode)?.title}</b></div><div><span>运行方案</span><b>{methodOptions.find((item) => item.value === method)?.label}</b></div><div><span>客户端参与</span><b>{participationRate}%</b></div><div><span>训练轮次</span><b>{rounds}</b></div><div><span>保护/防御</span><b>{mode === 'privacy' ? (privacyDefense ? '启用' : '关闭') : mode === 'backdoor' ? (backdoorDefense ? '启用' : '关闭') : '不适用'}</b></div></div>{scenarioId ? <div className="validation-pass"><CheckCircleFilled />场景快照已绑定</div> : <Alert type="warning" showIcon message="请先设置基础异构环境" action={<Button size="small" onClick={() => navigate('/scenario-analysis')}>前往设置</Button>} />}<Space direction="vertical" className="experiment-actions" style={{ width: '100%' }}><Button block icon={<DownloadOutlined />} onClick={() => exportExperimentConfig(config)}>导出当前配置</Button><Button type="primary" size="large" block icon={<RocketOutlined />} loading={submitting} disabled={!scenarioId || Boolean(capabilityError)} onClick={startExperiment}>创建并启动实验</Button></Space></Panel></aside>
    </div>
  </div>;
}
