...
RUN_ID="${RUN_ID:-officehome_60c_adaptive_defense}"
...
FEATURE_CACHE_DIR="${FEATURE_CACHE_DIR:-exp/headonly_system/cache/officehome_60c_adaptive_defense}"
...
DP_NOISE_MULTIPLIER="${DP_NOISE_MULTIPLIER:-0.1}"
...
LOCAL_DECOY_USE="${LOCAL_DECOY_USE:-1}"
LOCAL_DECOY_NUM_PER_CLASS="${LOCAL_DECOY_NUM_PER_CLASS:-40}"
...
"$PYTHON_BIN" - \
  ...
  "$FEATURE_EXTRACTION_LOCK_FILE" "$DP_ENABLED" "$ATTACK_TYPE" <<'PY'
import sys
...
for path in paths:
    ...
    ggeur['headonly_cache_version'] = (
        'officehome_60c_' + attack_type + '_defense' if dp_enabled
    ...
    cfg['expname'] = f'officehome_60c_{attack_type}_{variant}_{role}'
