...
def _load_head(run_dir):
    ...
    return torch.load(head_path, map_location='cpu'), trig_path
...
def _summarize(labels, preds, target_label):
    ...
    return dict(correct=correct, total=total, accuracy=correct / total if total else 0.0,
                asr=sum(hits), asrRate=(sum(hits) / total) if total else 0.0)
...
def run(spec):
    ...
    _plot(images, labels, attack_entry['poisoned'], class_names, triggered_path,
    ...
    if defense_entry:
        ...
        _plot(images, labels, defense_entry['poisoned'], class_names, defense_path,
