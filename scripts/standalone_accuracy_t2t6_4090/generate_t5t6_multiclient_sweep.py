"""Generate stratified 12/20-client candidates for T5/T6."""
...
plan = {
    "profile_name": "T5-T6 stratified multi-client sweep",
...
add_candidate("t5_mdsent_rnn", "s12_t20_lr1e4", 12, 20, 1e-4)
add_candidate("t5_mdsent_rnn", "s20_t20_lr1e4", 20, 20, 1e-4)
add_candidate("t5_mdsent_rnn", "s20_t50_lr1e4", 20, 50, 1e-4)
add_candidate("t5_mdsent_rnn", "s20_t50_proto_lr5e5_cov01", 20, 50,
              5e-5, 0.1, True)

add_candidate("t6_mdsent_lstm", "s12_t100_lr1e4", 12, 100, 1e-4)
add_candidate("t6_mdsent_lstm", "s20_t100_lr1e4", 20, 100, 1e-4)
add_candidate("t6_mdsent_lstm", "s20_t50_lr1e4", 20, 50, 1e-4)
add_candidate("t6_mdsent_lstm", "s20_t100_proto_lr5e5_cov01", 20, 100,
              5e-5, 0.1, True)

(HERE / "t5t6_multiclient_sweep.json").write_text(
    json.dumps(plan, ensure_ascii=False, indent=2) + "\n",
    encoding="utf-8",
)
print(HERE / "t5t6_multiclient_sweep.json")
